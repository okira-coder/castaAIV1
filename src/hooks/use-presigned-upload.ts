...
interface StorageInfo {
  type: "local" | "vercel-blob" | "s3" | "cloudinary";
  ...
  const upload = useCallback(
    async (
      ...
      try {
        ...
        if (supportsDirectUpload && storageType === "cloudinary") {
          // Request presigned URL for Cloudinary
          ...
          // Add all Cloudinary fields
          if (uploadUrlData.fields) {
            for (const [key, value] of Object.entries(uploadUrlData.fields)) {
              formData.append(key, value as string);
            }
          }

          const uploadResponse = await fetch(uploadUrlData.url, {
            method: uploadUrlData.method || "POST",
            body: formData,
          });
